...
Optional prerelease identifier for creating pre-release versions (e.g., alpha, beta, rc). Can be configured globally or per-package, and can be overridden via CLI flag.
...
With this configuration, version `1.0.0` would become `1.1.0-alpha.1` for a feature commit.

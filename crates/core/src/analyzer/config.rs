...
pub struct AnalyzerConfig {
    ...
    /// which will prevent these commit types from resulting in a version bump.
    ///
    /// Defaults to [`NAMED_PARSERS`]. An empty map classifies every commit
    /// into an unnamed group, so callers building this config by hand should
    /// supply a fully resolved set (see

...
impl Github {
    ...
    pub async fn new(
        ...
        let base_uri = format!("{}://api.{}", url.scheme, url.host);
        ...
        let repo = instance.repos(&url.owner, &url.name).get().await?;
        let err_msg = format!(
            "failed to find default branch for github repo: {}",
            url.path
        );
    ...
    async fn is_tag_ancestor_of_branch(
        ...
        let endpoint = format!(
            "{}/repos/{}/{}/compare/{}...{}",
            self.base_uri, self.url.owner, self.url.name, branch, tag_sha,
        );
...
impl Forge for Github {
    ...
    async fn get_latest_tag_for_prefix(
        ...
        while has_next_page {
            let vars = TagSearchQueryVariables {
                owner: self.url.owner.clone(),
                repo: self.url.name.clone(),
    ...
    async fn get_commits(
        ...
        while let Some(thin_commit) = stream.try_next().await? {
            ...
            let route = format!(
                "/repos/{}/{}/commits/{}",
                self.url.owner, self.url.name, thin_commit.sha
            );
    ...
    async fn create_commit(&self, req: CreateCommitRequest) -> Result<Commit> {
        ...
        let endpoint = format!(
            "{}/repos/{}/{}/git/refs/heads/{}",
            self.base_uri, self.url.owner, self.url.name, req.target_branch
        );
    ...
    async fn get_open_release_pr(
        ...
        let stream = self
            ...
            .head(format!("{}:{}", self.url.owner, req.head_branch))
            .send()
            .await?
            .into_stream(&self.instance);

...
impl CommitsCore {
    ...
    pub async fn fetch_additional_commits_for_prerelease_aggregation(
        ...
        if let Some(tag) = last_stable_tag {
            commits = self
                .forge
                .get_commits(
                    Some(self.orchestrator_config.base_branch.clone()),
    ...
    async fn collect_tags_for_packages(
        ...
        for (name, package) in self.package_configs.hash().iter() {
            ...
            let graduating_to_stable = tag
                .as_ref()
                .map(|t| {
                    // current tag has pre-release identifier but pre-release
                    // configuration is empty, indicating we are graduating from
                    // pre-release to stable version
                    !t.semver.pre.is_empty() && package.prerelease.is_none()
                })
                .unwrap_or_default();

...
mod tests {
    ...
    /// Increments a prerelease version (e.g., "1.0.0-alpha.1" -> "1.0.0-alpha.2").
    /// If the prerelease doesn't have a numeric suffix, appends ".1".
    fn increment_prerelease(version: &Version) -> Version {
        let mut new_version = version.clone();
        ...
        // Parse the prerelease parts
        let pre_str = version.pre.as_str();
        let pre_parts: Vec<&str> = pre_str.split('.').collect();

        if let Some(last) = pre_parts.last() {
            if let Ok(num) = last.parse::<u64>() {
                // Last part is numeric, increment it
                let mut new_pre_parts: Vec<String> = pre_parts
                    [..pre_parts.len() - 1]
                    .iter()
                    .map(|s| s.to_string())
                    .collect();
                new_pre_parts.push((num + 1).to_string());
                let new_pre_str = new_pre_parts.join(".");
                new_version.pre = Prerelease::new(&new_pre_str).unwrap();
            } else {
                // No numeric suffix, append ".1"
                let new_pre_str = format!("{}.1", pre_str);
                new_version.pre = Prerelease::new(&new_pre_str).unwrap();
            }
        }
...
- Initial release"#;
    ...
    fn test_increment_prerelease_with_numeric_suffix() {
        ...
        let result = increment_prerelease(&version);
        assert_eq!(result, Version::parse("1.0.0-alpha.2").unwrap());

        let version = Version::parse("2.3.4-beta.5").unwrap();
        let result = increment_prerelease(&version);
        assert_eq!(result, Version::parse("2.3.4-beta.6").unwrap());

        let version = Version::parse("1.0.0-rc.10").unwrap();
        let result = increment_prerelease(&version);
        assert_eq!(result, Version::parse("1.0.0-rc.11").unwrap());
    }
    ...
    fn test_increment_prerelease_without_numeric_suffix() {
        ...
        let version = Version::parse("2.3.4-beta").unwrap();
        let result = increment_prerelease(&version);
        assert_eq!(result, Version::parse("2.3.4-beta.1").unwrap());
    }

    #[test]
    fn test_increment_prerelease_with_complex_identifier() {
        let version = Version::parse("1.0.0-alpha.beta.1").unwrap();
        let result = increment_prerelease(&version);
        assert_eq!(result, Version::parse("1.0.0-alpha.beta.2").unwrap());

        let version = Version::parse("1.0.0-rc.1.2").unwrap();
        let result = increment_prerelease(&version);
        assert_eq!(result, Version::parse("1.0.0-rc.1.3").unwrap());
    }

    #[test]
    fn test_increment_prerelease_stable_version_unchanged() {

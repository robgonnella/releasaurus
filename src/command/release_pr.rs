...
use crate::{
    ...
    config::{
        Config,
        manifest::{
            load_additional_manifests_for_package,
            load_release_type_manifests_for_package,
        },
...
async fn gather_release_prs_by_branch(
    ...
    for pkg in releasable_packages.iter() {
        let mut file_changes =
            Framework::update_package(pkg, releasable_packages).await?;
...
async fn get_releasable_packages(
    ...
    for package in config.packages.iter() {
        ...
        if let Some(release) = analyzer.analyze(package_commits, current_tag)? {
            ...
            let manifest_files =
                load_release_type_manifests_for_package(forge_manager, package)
                    .await?;

            let additional_manifest_files =
                load_additional_manifests_for_package(forge_manager, package)
                    .await?;

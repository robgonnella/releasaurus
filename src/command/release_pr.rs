...
use crate::{
    ...
    cli,
    config::{self, CliSinglePackageConfig},
    forge::{
        config::{DEFAULT_PR_BRANCH_PREFIX, PENDING_LABEL, RemoteConfig},
        traits::Forge,
        types::{
...
fn process_package(
    ...
    let changelog = GitCliffChangelog::new(ChangelogConfig {
        ...
        package: PackageConfig {
            ...
            path: config.package.path.clone(),
            tag_prefix: config.package.tag_prefix.clone(),
        },
...
pub fn execute(args: &cli::Args) -> Result<()> {
    ...
    for single in cli_config {
        debug!(
            "processing changelog for package path: {}",
            single.package.path
        );
        let name = single.package.name.clone();
        let output = process_package(&single, remote_config)?;
        if name.is_empty() {
    ...
    let manifest_result = process_manifest(manifest, &git);

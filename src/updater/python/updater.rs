...
impl PythonUpdater {
    ...
    fn analyze_single_package(&self, package: &Package) -> Result<Package> {
        ...
        let (current_version, local_dependencies) = if manifest_path.file_name()
            ...
            == Some(std::ffi::OsStr::new("setup.py"))
        {
            ...
            == Some(std::ffi::OsStr::new("setup.cfg"))
        {
            self.analyze_setup_cfg(&manifest_path)?
        } else {
            (None, Vec::new())
        };
    ...
    fn analyze_pyproject_toml(
        &self,
        manifest_path: &Path,
    ) -> Result<(Option<String>, Vec<LocalDependency>)> {
        ...
        // Extract version and dependencies based on the build system
        let current_version =
            self.extract_version_from_pyproject(&pyproject, manifest_path)?;
        let local_dependencies =
            self.extract_dependencies_from_pyproject(&pyproject)?;

        Ok((current_version, local_dependencies))
    }
    ...
    fn analyze_setup_py(
        ...
        let current_version = setup_py.version.clone();
        let local_dependencies =
    ...
    fn analyze_setup_cfg(
        &self,
        _manifest_path: &Path,
    ) -> Result<(Option<String>, Vec<LocalDependency>)> {
        // For now, return empty - full setup.cfg parsing would require INI parser
        warn!("setup.cfg parsing not fully implemented yet");
        Ok((None, Vec::new()))
    }

    /// Extract version from pyproject.toml based on build system
    fn extract_version_from_pyproject(
        &self,
        pyproject: &PyProjectToml,
        manifest_path: &Path,
    ) -> Result<Option<String>> {
        // Try PEP 621 project.version first
        if let Some(ref project) = pyproject.project
            && let Some(ref version) = project.version
        {
            return Ok(Some(version.clone()));
        }

        // Try tool-specific version locations
        if let Some(ref tool) = pyproject.tool {
            // Poetry version
            if let Some(ref poetry) = tool.poetry {
                return Ok(Some(poetry.version.clone()));
            }

            // PDM version
            if let Some(ref pdm) = tool.pdm
                && let Some(ref version_value) = pdm.version
        ...
        // Try to extract from __init__.py if version is dynamic
        self.extract_version_from_init_py(manifest_path)
    }

    /// Extract version from __init__.py file
    fn extract_version_from_init_py(
        &self,
        manifest_path: &Path,
    ) -> Result<Option<String>> {
        let package_dir = manifest_path.parent().unwrap();
        // Look for common version patterns in __init__.py

        for init_path in &[
            package_dir.join("src").join("__init__.py"),
            package_dir.join("__init__.py"),
        ] {
            if init_path.exists()
                && let Ok(content) = fs::read_to_string(init_path)
                && let Some(version) =
                    self.extract_version_from_python_file(&content)
            {
    ...
    fn update_pyproject_toml(
        ...
        {
            project.version = Some(package.next_version.clone());
            info!(
        ...
        {
            poetry.version = package.next_version.clone();
            info!("Updated Poetry version to {}", package.next_version);
        }
    ...
    fn update_setup_py(
        ...
        let updated_content = version_pattern.replace_all(
            &content,
            format!(r#"version="{}""#, package.next_version).as_str(),
        );
        ...
        if updated_content != content {
            ...
            info!("Updated setup.py version to {}", package.next_version);
        }
...
mod tests {
    ...
    use crate::updater::{framework::Language, python::types::PythonMetadata};
...
"#,
        ...
        assert_eq!(analyzed_package.current_version, Some("0.1.0".to_string()));
        assert_eq!(analyzed_package.next_version, "0.2.0");
...
"#;
        ...
        assert_eq!(analyzed_package.current_version, Some("1.0.0".to_string()));
        assert_eq!(analyzed_package.next_version, "1.1.0");
...
"#;
        ...
        assert_eq!(analyzed_package.current_version, Some("2.0.0".to_string()));

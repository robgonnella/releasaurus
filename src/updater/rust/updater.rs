...
impl CargoUpdater {
    ...
    fn update_manifest_file(&self, package: &Package) -> Result<()> {
        ...
        if let Some(package_section) = &mut cargo_toml.package {
            package_section.version = package.next_version.semver.to_string();
            info!(
                "Updated package {} version to {}",
                package.name, package.next_version.semver
            );
        }
...
mod tests {
    use crate::{
        ...
        updater::{framework::Language, rust::types::RustMetadata},
    };
    ...
    fn test_create_single_package() {
        ...
        let semver_version = semver::Version::parse("1.0.0").unwrap();
        ...
        assert_eq!(package.next_version.semver, semver_version);
